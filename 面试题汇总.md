...
2. 401 Unauthorized具体见服务器怎么表示用户
...
2. 增加了断点续传。range：bytes=xxx。http1.0传输一个文件只能从文件的开头传输，http1.1可以从文件的中间位置传输。返回状态码为 206 partial content
...
```javascript
	...
	function judgeType(type) {
		return function(obj) {
			return Object.prototype.toString.call(obj) === `[Objetc ${type}]`
		}
	}
